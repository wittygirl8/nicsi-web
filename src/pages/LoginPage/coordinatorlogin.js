...
import './Login.css';
import {signin, AdminSignin, CandidateSignin} from '../../data-fetch/api.js';
...
export const CoordinatorLogin = () => {
    ...
    const [Coordinator_login, setCoordinator_login] = useState({
        email:"",
        pass:"",
        userType: "user"
    });
    ...
    const handleInput = (e) => {
        ...
        setCoordinator_login({ ...Coordinator_login, [name]: value});
    }
    ...
    const PostData = async (e) => {
        ...
        const { email, pass, userType } = Coordinator_login;
        if(email !==''){
            setemailError('');
        ...
        if(emailErrorRef.current == '' && passwordErrorRef.current == '') {
            ...
            var signinResponse = ''
            if(userType === "user") {
                signinResponse = await signin(requestData);
            }else if(userType === "admin") {
                signinResponse = await AdminSignin(requestData);
            }else if(userType === "candidate") {
                signinResponse = await CandidateSignin(requestData);
            }
            // dispatch(login({
            ...
            <div className="container-fluid">
                ...
                <div className="col-sm-6 text-black">
                    ...
                    <form style={{width: '23rem'}}>
                        <h3 className="fw-normal mb-3 pb-3" style={{letterSpacing: 1, color:'white'}}>Coordinator Sign In</h3>
                        {CredentialError&&<div className="err-credential-msg">{CredentialError}</div>}
                        <div className="form-outline mb-4">
                        <input type="text" className="form-control form-control-lg"  autoComplete="off" value={Coordinator_login.name} onChange={handleInput} name="email" id="email" placeholder="Your Email"/>
                        {emailError&&<div className="err-msg">{emailError}</div>}
                        </div>
                        <div className="form-outline mb-4">
                            <div class="input-group mb-3">
                                <input type={showPassword?'text':'password'} className="form-control form-control-lg" autoComplete="off"  aria-describedby="basic-addon1" value={Coordinator_login.pass} onChange={handleInput} name="pass" id="pass" placeholder="Password"/>
                        ...
                        <p className="small mb-5 pb-lg-2 text-center"><Link to='/admin-sign-in' className="signup-image-link link-info"> Admin Sign In </Link><Link to='/sign-in' className="signup-image-link link-info ml-2"> Candidate Sign In </Link></p>
